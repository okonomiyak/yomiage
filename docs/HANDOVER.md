...
コマンドは 24 個。グローバル登録（全サーバーで使える）。

| 分類 | コマンド |
| --- | --- |
| 読み上げ | `/join` `/leave` `/bind` `/unbind` `/skip` `/stats` |
| 自分の声 | `/voice` `/speed` `/pitch` `/intonation` |
| 音楽 | `/play` `/queue` `/next` `/stop` `/volume` `/dashboard` `/playlist` |
...
お気に入り（`/playlist`）は PLAN §13-10 参照。サーバー共有で、登録した名前と `/play` の引数が完全一致したときだけ URL に差し替える（一致しなければ従来通り URL/検索語として扱う）。

読み上げ統計（`/stats`）は PLAN §13-11 参照。文字数のみをサーバー×ユーザー単位で数え、当日と累計の 2 値だけ持つ（日別の履歴は保存しない）。

`/play` は YouTube の再生リスト・ニコニコのマイリスト/シリーズの URL も受け付ける（PLAN §13-12）。専用コマンドはなく、URL が再生リスト専用ページの形（`/playlist?list=...` や `/mylist/...` `/series/...`）と一致したときだけ自動でまとめて積む。最大 50 曲まで。曲ごとの長さは一覧取得時の簡易情報のみで、正確な値は実際に再生されるときに決まる。
...
- **Cookie の原本を yt-dlp に直接渡してはいけない。ここが一番痛い目を見た。**
  yt-dlp は終了時に**必ず** Cookie を書き戻すが、その書き戻しは **`expiry=0` の
  セッション Cookie を捨てる**。実機で原本が 29 行 → 27 行になり `mfa_session` が消え、
  その後ニコニコ側にセッションごと無効化されて**公開動画すら `Invalid session` で
  再生できなくなった**。`src/nicovideo.rs` の `cookie_copy()` で毎回コピーを渡し、
  書き戻しはコピーに捨てさせている
- **読み取り専用で渡すのも不可。** 書き戻しに失敗して yt-dlp がクラッシュし、
  `OSError: Read-only file system` で **抽出自体は成功しているのに終了コードだけ 1 になる**。
  なお `chmod 444` では防げない（コンテナが root で動いており、root は権限チェックを迂回する）
- **無効な Cookie は「無い」より悪い。** セッションが死んだ Cookie を渡すと、
  それまで問題なく再生できていた**公開動画まで `Invalid session` で失敗する**。
  ニコニコが急に全部再生できなくなったら、まず `data/nico_cookies.txt` を退けて試すこと
- **Cookie 用に専用の bind mount を足さないこと。** ファイルを置き忘れると
  Docker が**ディレクトリとして作ってしまう**。既存の `data/` の中に置く
