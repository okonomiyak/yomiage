...
- **Cookie ファイルを読み取り専用で渡してはいけない。** yt-dlp は終了時に必ず
  Cookie を書き戻すので、`:ro` マウントだと `OSError: Read-only file system` で落ちる。
  **抽出自体は成功しているのに終了コードだけ 1 になる**ので、原因が分かりにくい。
  `data/`（書き込み可能）に置くこと。なお `chmod 444` では防げない（コンテナが root で動いており、
  root は権限チェックを迂回する）。書き戻しはセッションの更新でもあり、期限が延びる利点がある
