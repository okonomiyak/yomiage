...
**変更 2026-07-26**: 「音楽再生機能」は非スコープから外し、実装した（利用者の要望による）。yt-dlp を使い、読み上げとは別トラックとして同じ VC に混ぜて流す。`/play` `/queue` `/next` `/stop` `/volume`。キューは songbird の `builtin-queue` に任せる（自前で持つと再生完了の検知と状態同期を書くことになりずれる）。読み上げは `play_input` で直接鳴らすためキューには入らず、音楽と独立して動く。音量はギルド単位で永続化する。読み上げに被せる前提なので既定音量は 30% と低めにしてある。
