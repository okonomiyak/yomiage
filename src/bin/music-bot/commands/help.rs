...
`/up_play <ファイル>` … 添付した MP3 をそのまま再生する（yt-dlp を介さない）
`/queue` … 再生中と待機中を見る
`/remove <番号>` … `/queue` の番号でキューから 1 曲取り除く（1 なら次の曲へ）
`/next` … 次の曲へ / `/stop` … 止めてキューを空にする
`/volume <0-100>` … 音量（サーバー単位）
`/stats` … 再生統計を見る（曲別・ユーザー別の再生回数）
...
・曲がキューで自動的に切り替わると、`/join` したチャンネルに次の曲を知らせる
　（手動 `/play` の直後は返信で分かるので知らせない）
