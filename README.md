...
- サーバー単位の読み上げ辞書
- yt-dlp を使った音楽再生（YouTube・ニコニコ動画。キュー対応。読み上げと同時に鳴らせる）。操作パネルには自動で進むシークバーが付く。ニコニコの会員限定動画は Cookie を置けば再生できる（[設定](#ニコニコの会員限定動画)）
...
省略可。ニコニコの公開動画はそのまま再生できるが、**会員限定・年齢制限・センシティブ指定の動画にはログイン状態が要る**。必要なら、ログイン済みの Cookie を Netscape 形式で書き出して `data/nico_cookies.txt` に置く。
...
Bot は**この原本を yt-dlp に直接渡さず、毎回コピーを渡す**。yt-dlp は終了時に必ず Cookie を書き戻すが、その書き戻しはセッション Cookie を捨ててしまい、原本が使うたびに削れていくため。置いたファイルはそのまま残る。

> **セッションが切れたら消すこと。** 無効になった Cookie を置いたままにすると、**それまで再生できていた公開動画まで `Invalid session` で失敗する**。ニコニコが急に全部再生できなくなったら、まず `data/nico_cookies.txt` を退けて試す。

Cookie には**アカウントのログイン状態がそのまま入っている**ので、リポジトリに入れたり共有したりしないこと（`data/` は `.gitignore` 済み）。
