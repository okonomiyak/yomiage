...
> **書き込み可能な場所に置くこと。** yt-dlp は終了時に必ず Cookie を書き戻すので、読み取り専用にすると抽出に成功していても終了コードが 1 になり、再生できなくなる。`data/` は書き込み可能でマウントしてあるのでそのまま使える。書き戻しはセッションの更新でもあり、置いたままにすると期限が延びる。
